...
├── .github/workflows/
│   ├── bootstrap-host.yml          ← one-time: install-k3s.sh + bootstrap-server.sh, via the runner
...
- **`.github/workflows/bootstrap-host.yml`** — runs `install-k3s.sh` + `bootstrap-server.sh` via the self-hosted runner instead of manual SSH. Requires passwordless `sudo` scoped to these two scripts (a host security decision only doable by hand, not automatable further) and, after it succeeds, a runner service restart before any other workflow runs — `bootstrap-server.sh`'s `usermod -aG cca` doesn't take effect for the already-running runner process. See `docs/RUNBOOK.md` §2.
